...
def _run_battery_decision_enablement_after_apply() -> None:
    """dtc-BatteryChargerDecisionStrategy is a REAL fed-sysml federation:
    fedtwin.json -> Event Registry, Step Function, Collector, Strategy Lambda,
    Feedback, IAM - all generated by Terraform. What fed-sysml cannot express is
    the hot-reader access (the Collector only fetches the triggering
    strategyAction's own inputParameters, and the Strategy role gets logs-only
    IAM), so this enablement supplies the dtcGrid/dtcBattery hot-reader env vars
    and the InvokeFunction permission to the FEDERATION-OWNED Strategy Lambda.
    It must run AFTER that Lambda exists (i.e. after apply).

    Fully isolated: any failure (twins not deployed, Strategy Lambda not yet
    applied, boto3/credentials missing) is reported and skipped, never raised, so
    it can never affect the other fed-sysml federations.
    """
    try:
        from scripts import federate_battery_charger_decision
    except ImportError:
        return

    try:
        federate_battery_charger_decision.main()
    except SystemExit as error:
        print(f"\nBattery decision enablement skipped: {error}")
    except Exception as error:  # noqa: BLE001 - must never break fed-sysml federations
        print(f"\nBattery decision enablement skipped due to an error: {error}")
...
def run_fed_sysml_terraform_apply_saved_plan_stage(config: PipelineConfig) -> None:
    ...
    _run_battery_decision_enablement_after_apply()
